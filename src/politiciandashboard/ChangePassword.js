...
function ChangePassword() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.put(`${config.url}/politician/changepassword`, {
        email,
        newPassword
      });
  ...
  return (
    <div className="politician-security-card-container">
      ...
      <form onSubmit={handleSubmit} className="politician-security-form">
        ...
        <input
          ...
          required
        />
        <button type="submit" className="politician-security-button">Update Password</button>
      </form>
    </div>
  );
}
